...
// Module-level lock — prevents concurrent sync runs (e.g. online event + SW
// SYNC_READINGS message firing simultaneously and double-submitting items).
let syncing = false
...
async function syncItem(
  ...
  await submitReading(
    ...
    item.sealNumber, item.tampered, item.idempotencyKey
  )
...
export async function syncPending(): Promise<number> {
  if (syncing) return (await listPending()).length
  syncing = true
  try {
    ...
    for (const item of items) {
      ...
      try {
        ...
        if (status === 409) {
          // Conflict — reading already exists (duplicate or read by another user)
          await saveConflict({
